...
namespace
{
  ...
  QRegExp wcall_alphabet {"[1-9]{0,1}[A-Za-z]{1,2}[0-9]{1,4}[A-Za-z]{1,6}"};
  QRegExp dxCall_alphabet {"[A-Za-z0-9/]*"};
  QRegExp dxGrid_alphabet {"[A-Ra-r]{2,2}[0-9]{2,2}[A-Xa-x]{2,2}[0-9]{2,2}[A-Xa-x]{2,2}"};
  ...
  QRegExpValidator call_valid(wcall_alphabet, 0);
...
MainWindow::MainWindow(bool multiple, QSettings * settings, QSharedMemory *shdmem,
  ...
  m_logqso73 {false},
  m_firstdecmycall {true},
  m_mycalldecoded {false},
  m_nonstdrcvd {false},
  m_rightbutton {""},
  m_repliedCQ {""},
  m_dxbcallTxHalted {""},
...
//  m_framesAudioInputBuffered (RX_SAMPLE_RATE / 10),
  m_framesAudioInputBuffered (RX_SAMPLE_RATE * 2),
...
//  setWindowTitle (tr ("JTDX v17.0      WSJT-X v1.7 based HF evaluation version by UA3DJY"));
  setWindowTitle (QApplication::applicationName () + tr (" v18.0  by HF community                                                     derivative work based on WSJT-X by K1JT"));

  createStatusBar();

  connect(&proc_jtdxjt9, SIGNAL(readyReadStandardOutput()),this, SLOT(readFromStdout()));
  connect(&proc_jtdxjt9, SIGNAL(error(QProcess::ProcessError)),this, SLOT(jt9_error(QProcess::ProcessError)));
  connect(&proc_jtdxjt9, SIGNAL(readyReadStandardError()),this, SLOT(readFromStderr()));
...
//  connect(&p3, SIGNAL(readyReadStandardOutput()),this, SLOT(p3ReadFromStdout()));
  ...
  ui->wantedCall->setValidator (new QRegExpValidator {dxCall_alphabet, this});
  ui->dxCallEntry->setValidator (new QRegExpValidator {dxCall_alphabet, this});
  ui->dxGridEntry->setValidator (new QRegExpValidator {dxGrid_alphabet, this});
  ...
  autoButtonTimer.setSingleShot(true);
  connect(&autoButtonTimer, &QTimer::timeout, this, &MainWindow::auto_button_off);
  ...
  auto_tx_label->setText (m_quick_call ? "AutoTx Armed" : "AutoTx Disarmed");
...
//  Q_EMIT startAudioInputStream (m_config.audio_input_device (), m_framesAudioInputBuffered, &m_detector, m_downSampleFactor, m_config.audio_input_channel ());
  ...
  if(m_mode=="T10") on_actionT10_triggered();
  ...
  m_showMyCallMsgRxWindow=ui->actionShow_messages_with_my_callsign_in_RX_frequency_window->isChecked();
...
void MainWindow::writeSettings()
{
  ...
  m_settings->setValue("CQ/DX",m_cqdx);  
  m_settings->setValue("DXcall",ui->dxCallEntry->text());
  m_settings->setValue("DXgrid",ui->dxGridEntry->text());
  m_settings->setValue("WantedCall",ui->wantedCall->text());
  ...
  m_settings->setValue("ShowMyCallMessagesRxWindow",ui->actionShow_messages_with_my_callsign_in_RX_frequency_window->isChecked());
  ...
  m_settings->setValue("QuickCall", m_quick_call);
...
void MainWindow::readSettings()
{
  ...
  ui->wantedCall->setText(m_settings->value("WantedCall","").toString());
  ...
  if(m_rrr) {
    ...
    m_cqdx = m_settings->value("CQ/DX",false).toBool();
  ui->dxCheckBox->setChecked(m_cqdx);
  ...
  m_settings->beginGroup("Common");
  m_mode=m_settings->value("Mode","JT9+JT65").toString();
  m_modeTx=m_settings->value("ModeTx","JT65").toString();
  ...
  ui->actionShow_messages_with_my_callsign_in_RX_frequency_window->setChecked(m_settings->value("ShowMyCallMessagesRxWindow",true).toBool());
...
//  ui->FTol_combo_box->setCurrentText(m_settings->value("FTol","500").toString ());
  ...
  m_lockTxFreq=m_settings->value("LockTxFreq",true).toBool();
  ...
  if(m_filter) dec_data.params.nfilter=1;
  if(!m_filter) dec_data.params.nfilter=0;
  ui->filterButton->setChecked(m_filter);
  m_agcc=m_settings->value("AGCcompensation",false).toBool();
  m_hint=m_settings->value("Hint",false).toBool();
  m_disable_TX_on_73=m_settings->value("73TxDisable",false).toBool();
  m_quick_call=m_settings->value("QuickCall",false).toBool();
  m_autoseq=m_settings->value("AutoSequence",false).toBool();
  if(m_agcc) dec_data.params.nagcc=1;
  if(!m_agcc) dec_data.params.nagcc=0;
  if(m_hint) dec_data.params.nhint=1;
  if(!m_hint) dec_data.params.nhint=0;
  ui->AGCcButton->setChecked(m_agcc);
  ui->hintButton->setChecked(m_hint);
  ui->disableTx73Button->setChecked(m_disable_TX_on_73);
  ui->AutoTxButton->setChecked(m_quick_call);
  ui->AutoSeqButton->setChecked(m_autoseq);
  m_swl=m_settings->value("SWLMode",false).toBool();
  if(m_swl) dec_data.params.nswl=1;
  if(!m_swl) dec_data.params.nswl=0;  
...
//  int k (frames - 1);
  ...
  if(m_diskData) dec_data.params.ndelay=0;

  if(ihsym == m_hsymStop || (m_mode.mid(0,4)!="WSPR" && dec_data.params.ndelay > 0 && (ihsym+int(float(dec_data.params.ndelay)*3.38)) > m_hsymStop)) {
    if(m_mode=="Echo") {
      float snr=0;
      int nfrit=0;
      int nqual=0;
...
//Don't restart Monitor after an Echo transmission
    ...
    if(!m_diskData) {                        //Always save; may delete later
      ...
      if ((m_saveAll || m_saveDecoded || m_mode.mid (0,4) == "WSPR") && m_fname != "")
      ...
      if (m_mode.mid (0,4) == "WSPR") {
        ...
        if (err!=0) msgBox("Error saving c2 file"+c2name_string);
      }
    }
  ...
  if (QDialog::Accepted == m_config.exec ())
    {
      ...
      bool b=m_config.enable_VHF_features() and (m_mode=="JT4" or m_mode=="JT65" or
                              m_mode=="ISCAT" or m_mode=="JT9" or m_mode=="JTMSK" or m_mode=="T10");
      VHF_controls_visible(b);
    }
...
if (checked)    
 {
	 dec_data.params.nfilter=1;
	 m_filter=true;
     m_wideGraph->setFilter(m_filter);
 }
 if (!checked)    
{
	 dec_data.params.nfilter=0;
	 dec_data.params.nagainfil=0;
	 m_filter=false;
     m_wideGraph->setFilter(m_filter);
 }
...
if (checked)    
 {
	 dec_data.params.nswl=1;
	 m_swl=true;
 }
 if (!checked)    
{
	 dec_data.params.nswl=0;
	 m_swl=false;
 }
}
...
if (checked)    
 {
	 dec_data.params.nagcc=1;
	 m_agcc=true;
 }
 if (!checked)    
{
	 dec_data.params.nagcc=0;
	 m_agcc=false;
 }
}
...
void MainWindow::on_hintButton_clicked (bool checked)
{
  if (checked)    
  {
     dec_data.params.nhint=1;
     m_hint=true;
  }
  if (!checked)    
  {
     dec_data.params.nhint=0;
     m_hint=false;
  }
  dec_data.params.nstophint=1; 
  if ((m_modeTx=="JT65" || m_modeTx=="JT9" || m_modeTx=="T10") && m_hint)
  {
    dec_data.params.nstophint=0; //let passing through non-CQ messages from Call3 based Hint RX freq decoder 
    stophintTimer.start(314000); //((14+5*60)*1000) block in 5 minutes
  }
}

void MainWindow::on_disableTx73Button_clicked (bool checked)
{
  if (checked)    
  {
     m_disable_TX_on_73=true;
  }
  else    
  {
     m_disable_TX_on_73=false;
...
void MainWindow::monitor (bool state)
{
  ...
  if (state) {
    ...
    if (!m_monitoring) {
      ...
      if (currentTime.second() > 0 && currentTime.second() < 35) dec_data.params.ndelay=currentTime.second()+m_addtx; // 2 second processing delay
      m_addtx=0;
    }
  } else {
...
{
  m_auto = checked;
  if(!m_auto) m_QSOProgressTX = NONE;
  m_messageClient->status_update (m_freqNominal, m_mode, m_hisCall,
                                  QString::number (ui->rptSpinBox->value ()),
                                  m_modeTx, ui->autoButton->isChecked (),
...
void MainWindow::auto_button_off ()
{
	auto_tx_mode (false);
}
...
void MainWindow::createStatusBar()                           //createStatusBar
{
  ...
  tx_status_label->setAlignment(Qt::AlignHCenter);
  tx_status_label->setAlignment(Qt::AlignVCenter);
  tx_status_label->setContentsMargins(1,1,1,1);
  ...
  auto_tx_label->setAlignment(Qt::AlignVCenter);
  auto_tx_label->setContentsMargins(1,1,1,1);
  auto_tx_label->setMinimumSize(QSize(104,20));
  ...
  qso_count->setAlignment(Qt::AlignVCenter);
  qso_count->setContentsMargins(1,1,1,1);
  qso_count->setMinimumSize(QSize(100,20));
...
void MainWindow::diskDat()                                   //diskDat()
{
  ...
  float db=0.0; //m_config.degrade();
//  if(db > 0.0) degrade_snr_(dec_data.d2,&dec_data.params.kin,&db);
  if(db > 0.01) degrade_snr_(dec_data.d2,&dec_data.params.kin,&db);
...
void MainWindow::on_actionShow_messages_with_my_callsign_in_RX_frequency_window_toggled(bool checked)
{
  if(checked) {
     m_showMyCallMsgRxWindow=true;
  }
  else {
     m_showMyCallMsgRxWindow=false;
  }
}

void MainWindow::on_actionBypass_text_filters_on_RX_frequency_toggled(bool checked)
{
  if(checked) {
     m_bypassRxfFilters=true;
  }
  else {
     m_bypassRxfFilters=false;
  }
...
void MainWindow::on_actionEnable_main_window_popup_toggled(bool checked)
{
  if(checked) {
     m_windowPopup=true;
  }
...
//  dec_data.params.ntxmode=9;
  if(m_modeTx=="JT9") dec_data.params.ntxmode=9;
...
//  }
  ...
  if(m_mode=="ISCAT" or m_mode=="JTMSK" or m_bFast9) {
    ...
    watcher3.setFuture (QtConcurrent::run (std::bind (fast_decode_,&d2b[0],&narg[0],&m_msg[0][0],80)));
...
void::MainWindow::fast_decode_done()
{
  ...
  for(int i=0; i<100; i++) {
    ...
    if(((m_mode=="JTMSK") or m_bFast9) and tmax>=0.0 and i1>10 and
...
//    if (decodedtext.messageContainsCall(m_baseCall)) {
...
// Write decoded text to file "ALL.TXT".
    ...
    if (f.open(QIODevice::WriteOnly | QIODevice::Text | QIODevice::Append)) {
      QTextStream out(&f);
      if(m_RxLog==1) {
        out << QDateTime::currentDateTimeUtc().toString("yyyyMMdd hh:mm")
            << "  " << qSetRealNumberPrecision (12) << (m_freqNominal / 1.e6) << " MHz  "
            << m_mode << endl;
        m_RxLog=0;
      }
      int n=message.length();
      out << QDateTime::currentDateTimeUtc().toString("yyyyMMdd ") << message.mid(0,n-2) << endl;
      f.close();
    } else {
...
{
  QByteArray t=proc_jtdxjt9.readAllStandardError();
  msgBox(t);
}
...
void MainWindow::readFromStdout()                             //readFromStdout
{
  while(proc_jtdxjt9.canReadLine()) {
    ...
    if(t.indexOf("<DecodeFinished>") >= 0) {
      ...
      m_notified=false;
	  if (!m_mycalldecoded) {
	  	m_qsohistory.prepend("NONERCVD");
		m_qsohistory.removeLast();
	  }
	  m_mycalldecoded = false;
	  m_repliedCQ = "";
//msgBox(m_qsohistory.join(",")); //comment it  
//printf("m_qsohistory:%s\n",m_qsohistory.join(",").toStdString().c_str());
      return;
    } else {
      QFile f {m_dataDir.absoluteFilePath ("ALL.TXT")};
      if (f.open(QIODevice::WriteOnly | QIODevice::Text | QIODevice::Append)) {
        ...
        int n=t.length();
        out << QDateTime::currentDateTimeUtc().toString("yyyyMMdd ") << t.mid(0,n-2) << endl;
        f.close();
      } else {
        msgBox("Cannot open \"" + f.fileName () + "\" for append:" + f.errorString ());
      }
        ...
        if (m_config.insert_blank () && m_blankLine)
          {
            ...
            if (QDateTime::currentMSecsSinceEpoch() / 1000 - m_secBandChanged > 50)
              ...
              QString blnklinetime = QDateTime::currentDateTimeUtc().toString("hh:mm' UTC '");
              ui->decodedTextBrowser->insertLineSpacer ("------- " + blnklinetime + band.rightJustified (15, '-') + "--------");
	  ...
	  QString dxbasecall = Radio::base_callsign (ui->dxCallEntry->text ());
	  QString dxbcall = "";
	  if (dxbasecall != "") {
		dxbcall = dxbasecall;
		} else {
			if (m_hisCallOld != "")
				dxbcall = Radio::base_callsign (m_hisCallOld);
	  }
	  if (m_baseCall != "" && m_baseCall == Radio::base_callsign (decodedtext.call())) {
		  mycallinmsg = true;
		  m_mycalldecoded = true;
		  if(message_words.length () == 4) {
			if (dxbcall != "") {
				if (message_words.at (2).contains (dxbcall)) {
					if (message_words.at (3).contains (QRegularExpression {"(R\\+|R-)"}))
						m_qsohistory.prepend (dxbcall + " RRPRTRCVD_" + message_words.at (3));
					else if (message_words.at (3).contains (QRegularExpression {"(\\+|-)"}))
						m_qsohistory.prepend (dxbcall + " REPORTRCVD_" + message_words.at (3));
					else if (message_words.at (3).contains (QRegularExpression {"(RRR|RR73|73)"}))
						m_qsohistory.prepend (dxbcall + " " + message_words.at (3) + "RCVD");
					else if (message_words.at (3).contains (QRegularExpression {"([A-Z]+[A-Z]+[0-9]+[0-9]+)"}))
						m_qsohistory.prepend (dxbcall + " GRIDRCVD");
					else {
						m_qsohistory.prepend (dxbcall + " FREEMSG");
					}
					m_qsohistory.removeLast();
				}
		    } else if(m_QSOProgressTX == CALLINGTX) { // m_ntx=7 may crash CALLINGTX value, need to change m_ntx functionality
				if (message_words.at (3).contains (QRegularExpression {"([A-Z]+[A-Z]+[0-9]+[0-9]+)"})) {
					m_currentQSOcallsign = Radio::base_callsign (message_words.at (2));
					m_repliedCQ.append (m_currentQSOcallsign + " REPLIEDCQGRID ");
					m_dateTimeQSOOn = QDateTime::currentDateTimeUtc();
				}
				else if (message_words.at (3).contains (QRegularExpression {"(\\+|-)"})) { // got reply to CQ with report
					m_currentQSOcallsign = Radio::base_callsign (message_words.at (2));
					m_repliedCQ.append (m_currentQSOcallsign + " REPLIEDCQRPRT ");
					m_dateTimeQSOOn = QDateTime::currentDateTimeUtc();
				}
				else if (message_words.at (3).contains (QRegularExpression {"(RRR|RR73|73)"}))
					m_repliedCQ.append (Radio::base_callsign (message_words.at (2)) + " " + message_words.at (3) + "RCVD ");
				if (m_firstdecmycall) {
					m_qsohistory.prepend(m_repliedCQ);
					m_qsohistory.removeLast();
					m_firstdecmycall = false;
				} else {
					m_qsohistory.removeFirst();
					m_qsohistory.prepend (m_repliedCQ);	
				}
			}
			  else if (m_config.clear_DX () && m_QSOProgressTX == ROGERSTX) {
				 if (message_words.at (3).contains (QRegularExpression {"(RRR|RR73|73)"}))
					m_qsohistory.prepend (Radio::base_callsign (message_words.at (2)) + " " + message_words.at (3) + "RCVD ");
					m_qsohistory.removeLast();
			}
		  }
	  }

      if (!m_notified && m_wantedCall.length() > 2 && t.mid(22).indexOf(" " + m_wantedCall) > 0) {
          if (m_windowPopup) {
			 this->showNormal();
			 this->raise();
			 QApplication::setActiveWindow(this);
...
//      }
      ...
      if (qAbs(decodedtext.frequencyOffset() - m_wideGraph->rxFreq()) <= 10 || (m_showMyCallMsgRxWindow && mycallinmsg) || baveJT4msg) {
        if(m_bypassRxfFilters || dec_data.params.nagain==1 || dec_data.params.nagainfil==1) {bypassRxfFilters = true;}
          // This msg is within 10 hertz of our tuned frequency, or a JT4 avg

// autoseq scenario where '73' free message with no mycall is received in 'replying to CQ' operation
		if(m_autoseq && (decodedtext.isNonStd1() || decodedtext.isNonStd2()) && !tcut.contains (m_baseCall) && tcut.contains ("73"))
			m_nonstdrcvd = true;
...
//        }
	  ...
	  auto skipmsgprocessing = false;
	  // Halt Tx if operator I called answered to other operator
	  if(tcut.length() >= 10 && m_auto && !decodedtext.isNonStd2()) {
		if (m_config.halttxreplyother() || m_autoseq) {
			if((m_QSOProgress == REPLYING || (m_skipTx1 && m_QSOProgress == REPORT))){
				if(message_words.length() == 4){ // three words(max index 3) in the message where "CQ DX" is counted as single word
					if(message_words.at (2).contains (Radio::base_callsign (ui->dxCallEntry->text ()))) {
						if ( message_words.at (1) != "DE" && message_words.at (1) != "QRZ"
							&& message_words.at (1) != "CQ" 
							&& !message_words.at (1).contains (QRegularExpression {"^CQ "})
							&& !message_words.at (1).contains (m_baseCall)
							&& !message_words.at (3).contains (QRegularExpression {"(RRR|RR73)"})
							&& message_words.at (3) != "73") {
							skipmsgprocessing = true;
							if (!tuneButtonTimer.isActive())
								tuneButtonTimer.start(50); //on_stopTxButton_clicked: turn Enable TX off and halt ongoing transmission
							if (!logClearDXTimer.isActive() && m_autoseq) 
								logClearDXTimer.start (100);
						}
					}
				}
			}
		...
		// Halt TX if the same report msg received n times
		if(m_config.halttxsamemsgrprt() && mycallinmsg && dxbcall != "" && tcut.contains(dxbcall) && m_qsohistory.at (0).contains ("RTRCVD_")) {
			int ncounter = m_config.nhalttxsamemsgrprt() - 1;
			int ncfm = 0;
			while (ncounter > 0) {
				if (m_qsohistory.at (2*ncounter).contains (dxbcall) && m_qsohistory.at (2*ncounter).contains ("RTRCVD_")
					&& m_qsohistory.at (2*(ncounter-1)).contains (dxbcall) && m_qsohistory.at (2*(ncounter-1)).contains ("RTRCVD_")) {
					if (m_qsohistory.at (2*ncounter) == m_qsohistory.at (2*(ncounter-1))) {
						ncfm++;
					} else {
						QStringList list_2 = m_qsohistory.at (2*ncounter).split(" ");
						QStringList list_1 = m_qsohistory.at (2*(ncounter-1)).split(" ");
						int index_2 = list_2.indexOf(dxbcall,0);
						int index_1 = list_1.indexOf(dxbcall,0);
						if (list_2.at(index_2+1) == list_1.at(index_1+1))
							ncfm++;
					}
				}
				ncounter--;
			}
			if (ncfm == m_config.nhalttxsamemsgrprt() - 1) {
				skipmsgprocessing = true;
				m_dxbcallTxHalted = dxbcall;
				if (!dxbcallTxHaltedClearTimer.isActive())
					dxbcallTxHaltedClearTimer.start (370000); // 6 minute + 10 seconds guard timer to prevent AutoSeq processing
				if (!tuneButtonTimer.isActive()) {
					tuneButtonTimer.start(50); //on_stopTxButton_clicked: turn Enable TX off and halt ongoing transmission
				} else {
					tuneButtonTimer.stop();
					tuneButtonTimer.start(50);
				}
				if (m_autoseq) { 
		...
		// come back to CQ if there is no answer to my report msg decoded n times
		if (m_config.backtocq() && m_QSOProgress == REPORT && m_QSOProgressTX == REPORTTX) {
			if(reportRCVD(message_words)) m_norprtrcvdCQ=0;
		}
		// if report msg to me is not decoded n times then Halt Tx
		if (m_config.retransmitmsg()) { 
			if(!m_skipTx1) {
				if(m_QSOProgress==REPLYING && m_QSOProgressTX==REPLYINGTX) {
					if(reportRCVD(message_words)) m_norprtrcvdRCQ = 0;
				}
			}
			else {
				if(m_QSOProgress==REPORT && m_QSOProgressTX==REPORTTX) {
					if(rReportRCVD(message_words)) m_norprtrcvdRCQ = 0;
				}
			}
		}
	  }
	  ...
	  // auto sequencing
	  if (m_autoseq && m_mode != "FT8" && !skipmsgprocessing && !tcut.contains ("/")) {
		  
		if (message_words.length() == 4) {
			if (mycallinmsg && message_words.at (2) != m_dxbcallTxHalted) {
				if (dxbasecall != "") {
					// common scenario while using standard messages in QSO
					if (message_words.at (2) == dxbasecall) {
						processMessage(t, 40, false);
					} else {
					// if 'MYCALL TNX 73' style message is being used instead of the standard one in 'replying to CQ' scenario
						if ((tcut.contains (" TU ") || tcut.contains (" TNX") || tcut.contains (" TKS") || tcut.contains ("73")) &&
							m_qsohistory.at (0).contains (dxbasecall + " RREPORTTXD")) {
							if (!tx73ButtonTimer.isActive())
								tx73ButtonTimer.start(0);
						}
					}
				} else {
				// processing only grid and report repeat messages if DX Call window is empty, for instance was cleaned by sending '73' message
				// 'mycall hiscall R-01' -> 'hiscall mycall RR73' -> 'mycall hiscall R-01' processing
					if (m_qsohistory.at (2).contains (message_words.at (2)) && !message_words.at (3).contains (QRegularExpression {"(RRR|RR73|73)"}))
						processMessage(t, 40, false);
				}
			}
		} else {
		// if 'MYCALL TNX73' or 'MYCALLTNX73' style message is being used instead of the standard one in 'replying to CQ' scenario
		// 'hiscall mycall R-01' -> 'MYCALL TNX73' or 'MYCALLTNX73'
			if (dxbasecall != "") {
				if (((message_words.length() == 3 && mycallinmsg) || (message_words.length() == 2 && m_baseCall != "" && tcut.contains(m_baseCall)))) {
					if (tcut.contains ("73") && m_qsohistory.at (0).contains (dxbasecall + " RREPORTTXD")) {
						if (!tx73ButtonTimer.isActive())
							tx73ButtonTimer.start(0);
					}
				}
      ...
      if(!m_config.prevent_spotting_false () or (m_config.prevent_spotting_false () and !decodedtext.isWrong ())) {
...
void MainWindow::on_cleanButton_clicked()                          //Erase
{
    clearDX ();
}
...
void MainWindow::guiUpdate()
{
  ...
  if(m_modeTx=="T10")  txDuration=1.0 + 85.0*m_nsps/12000.0;  // T10
...
//----------------------------------------------------------------------
    ...
    if(m_tune or m_mode=="Echo") {
      ...
      if(m_mode=="ISCAT") {
        ...
        if(m_modeTx=="T10") gen10_(message, &ichk, msgsent, const_cast<int *> (itone),
                                  &m_currentMessageType, len1, len1);								  
    ...
    last_tx_label->setText("LastTx: " + m_currentMessage);
    ...
    if (is_73) {
		if (m_disable_TX_on_73) {
			if (!autoButtonTimer.isActive())
				autoButtonTimer.start (100);
		}
    }
  ...
  if (g_iptt == 1 && iptt0 == 0)
    {
	  ...
	  if (m_QSOProgress == CALLING && ui->dxCallEntry->text() != "") {
		ui->dxCallEntry->setText("");
		ui->dxCallEntry->setStyleSheet("background-color: rgb(255,255,255);");
...
//Once per second:
  if(nsec != m_sec0) {
	...
	if(m_transmitting && m_mode.mid(0,4) != "WSPR") {
		if(m_QSOProgress==CALLING) m_QSOProgressTX=CALLINGTX;
		else if (m_QSOProgress==REPLYING) m_QSOProgressTX=REPLYINGTX;
		else if (m_QSOProgress==REPORT) m_QSOProgressTX=REPORTTX;
		else if (m_QSOProgress==ROGER_REPORT) m_QSOProgressTX=ROGER_REPORTTX;
		else if (m_QSOProgress==ROGERS) m_QSOProgressTX=ROGERSTX;
		else if (m_QSOProgress==SIGNOFF) m_QSOProgressTX=SIGNOFFTX;

		QString dxbasecall = Radio::base_callsign (ui->dxCallEntry->text ());

		if(nsec % 60 == 0) { // increase timer value if TX is incorrectly halted
		
        // autoseq scenario where '73' free message with no mycall is received in 'replying to CQ' operation
		if (m_autoseq && m_nonstdrcvd && m_QSOProgressTX == ROGER_REPORTTX && m_qsohistory.at (0).contains ("NONERCVD") 
		&& m_qsohistory.at (1).contains (dxbasecall + " RREPORTTXD")) {
			if (!tx73ButtonTimer.isActive()) 
				tx73ButtonTimer.start(250);
		}
			// come back to CQ if there is no answer to my report msg decoded n times
			if(m_QSOProgressTX==REPORTTX && dxbasecall != "") {
				if(m_config.backtocq()) { 
					auto nbacktocq = m_config.nbacktocq();
					++m_norprtrcvdCQ;
					if(m_norprtrcvdCQ > nbacktocq && m_qsohistory.at (nbacktocq*2).contains (dxbasecall + " REPLIEDCQ")) {
						cqButtonTimer.start(250); //on_pbCallCQ_clicked
						m_norprtrcvdCQ = 0;
					}
				}
			} else {
			...
			// if report msg to me is not decoded n times then Halt Tx
			if(m_config.retransmitmsg() && dxbasecall != "") { 
				auto nretransmitmsg = m_config.nretransmitmsg();
				++m_norprtrcvdRCQ;
				if(!m_skipTx1) {
					if(m_QSOProgressTX==REPLYINGTX) {
						if(m_norprtrcvdRCQ > nretransmitmsg && m_qsohistory.at (nretransmitmsg*2-1).contains (dxbasecall + " GRIDTXD")) {
							if (!tuneButtonTimer.isActive())
								tuneButtonTimer.start(250); //on_stopTxButton_clicked: turn Enable TX off and halt ongoing transmission
							if (!logClearDXTimer.isActive() && m_autoseq) 
								logClearDXTimer.start ((qAbs(m_TRperiod-m_nseq))*1000);
							m_norprtrcvdRCQ = 0;
						}
					} else {
						m_norprtrcvdRCQ = 0;
					...
					if(m_QSOProgressTX==REPORTTX) {
						if(m_norprtrcvdRCQ > nretransmitmsg && !m_qsohistory.at (nretransmitmsg*2).contains (dxbasecall + " REPLIEDCQ") 
							&& m_qsohistory.at (nretransmitmsg*2-1).contains (dxbasecall + " REPORTTXD")) {
							if (!tuneButtonTimer.isActive())
								tuneButtonTimer.start(250); //on_stopTxButton_clicked: turn Enable TX off and halt ongoing transmission
							if (!logClearDXTimer.isActive() && m_autoseq) 
								logClearDXTimer.start ((qAbs(m_TRperiod-m_nseq))*1000);
			...
			// Halt TX if the same RRR/RR73/73 msg received n times
			if(m_config.halttxsamemsg73() && (m_qsohistory.at (0).contains ("RRRRCVD") || m_qsohistory.at (0).contains ("73RCVD"))) { 
				int ncounter2 = m_config.nhalttxsamemsg73() - 1;
				int ncfm2 = 0;
				while (ncounter2 > 0) {
					if ((m_qsohistory.at (2*ncounter2).contains ("RRRRCVD") || m_qsohistory.at (2*ncounter2).contains ("73RCVD"))
						&& (m_qsohistory.at (2*(ncounter2-1)).contains ("RRRRCVD") || m_qsohistory.at (2*(ncounter2-1)).contains ("73RCVD"))) {
						if (m_qsohistory.at (2*ncounter2) == m_qsohistory.at (2*(ncounter2-1))) {
							ncfm2++;
						}
					}
					ncounter2--;
				}
				if (ncfm2 == m_config.nhalttxsamemsg73() - 1) {
					if (!tuneButtonTimer.isActive())
						tuneButtonTimer.start(250); //on_stopTxButton_clicked: turn Enable TX off and halt ongoing transmission
					if (!logClearDXTimer.isActive()) 
						logClearDXTimer.start (300);
				}
			}

		}
		// ROGERS + CQ dual message TX functionality
		if(m_mode != "FT8" && m_rightbutton == "CQ" && m_nlasttx == 4) {
			int nhalfTx=24;
			if(m_modeTx=="T10" || m_modeTx=="JT9") nhalfTx=25;
			if(nsec % 60 == nhalfTx) {
				on_pbCallCQ_clicked();
				m_rightbutton == "";
				if(m_rrr) {
		...
		if(m_mode != "FT8" && nsec % 60 == 46) {
			
			m_nonstdrcvd = false;
/*			
if(m_QSOProgressTX == REPLYINGTX) msgBox("REPLYINGTX");
if(m_QSOProgressTX == REPORTTX) msgBox("REPORTTX");
if(m_QSOProgressTX == ROGER_REPORTTX) msgBox("ROGER_REPORTTX");
if(m_QSOProgressTX == ROGERSTX) msgBox("ROGERSTX");
if(m_QSOProgressTX == SIGNOFFTX) msgBox("SIGNOFFTX");
if(m_QSOProgressTX == CALLINGTX) msgBox("CALLINGTX");
*/			
			
			if(m_QSOProgressTX==CALLINGTX || m_msgSent0.startsWith ("CQ ")) { // workaround for m_ntx = 7, messages generated in tab 2
				m_qsohistory.clear ();
				m_qsohistory << "" << "" << "" << "" << "" << "" << "" << "" << "" << "" << "" << "" << "" << "";
				m_qsohistory.prepend ("CQTXD");
			}
			else if (m_QSOProgressTX==REPLYINGTX) {
				m_qsohistory.clear ();
				m_qsohistory << "" << "" << "" << "" << "" << "" << "" << "" << "" << "" << "" << "" << "" << "";
				m_qsohistory.prepend (dxbasecall + " GRIDTXD");
				m_currentQSOcallsign = dxbasecall;
				auto currenttime = QDateTime::currentDateTimeUtc();
				if (m_mode != "FT8") {
					m_dateTimeQSOOn = currenttime.addSecs (-46);
				} else {
					m_dateTimeQSOOn = currenttime.addSecs (-12);
				}
			}
			else if (m_QSOProgressTX==REPORTTX) {
				if (m_skipTx1 && !m_qsohistory.at (1).contains (dxbasecall) && !m_qsohistory.at (2).contains (dxbasecall + " REPORTTXD")) {
					// replying to CQ with report message scenario
					m_qsohistory.clear ();
					m_qsohistory << "" << "" << "" << "" << "" << "" << "" << "" << "" << "" << "" << "" << "" << "";
				}
				m_qsohistory.prepend (dxbasecall + " REPORTTXD");
				if (m_currentQSOcallsign != dxbasecall) {
				auto currenttime = QDateTime::currentDateTimeUtc();
					if (m_mode != "FT8") {
						m_dateTimeQSOOn = currenttime.addSecs (-46);
					} else {
			...
			else if (m_QSOProgressTX==ROGER_REPORTTX) {
				m_qsohistory.prepend (dxbasecall + " RREPORTTXD");
				if (m_currentQSOcallsign != dxbasecall) {
					auto currenttime = QDateTime::currentDateTimeUtc();
					if (m_mode != "FT8") {
						m_dateTimeQSOOn = currenttime.addSecs (-46);
			...
			else if (m_QSOProgressTX==ROGERSTX) {
				m_qsohistory.prepend (dxbasecall + " RR73TXD");
				if (m_autoseq) { 
					if (!logClearDXTimer.isActive()) {
						int delay1 = 0;
						if (m_mode != "FT8") {
						...
						logClearDXTimer.start ((delay1 + (qAbs(m_TRperiod-m_nseq)))*1000);
					}
					if (m_auto && !m_qsohistory.contains ("CQTXD") && !autoButtonTimer.isActive())
						autoButtonTimer.start (4000);
				}
				
			}
			else if (m_QSOProgressTX==SIGNOFFTX) {
				m_qsohistory.prepend (dxbasecall + " 73TXD");
				if (m_autoseq) { 
					if (!logClearDXTimer.isActive()) {
						int delay2 = 0;
						if (m_mode != "FT8") {
						...
						logClearDXTimer.start ((delay2 + qAbs(m_TRperiod-m_nseq))*1000);
					}
					if (m_auto && !m_qsohistory.contains ("CQTXD") && !autoButtonTimer.isActive())
						autoButtonTimer.start (4000);
				}
...
//printf("m_qsohistory:%s\n",m_qsohistory.join(",").toStdString().c_str());
    ...
    QDateTime t = QDateTime::currentDateTimeUtc();
    QString currentDate = t.date().toString("dd MMM yyyy");
    QString utc = t.time().toString();
    QString hour = t.time().toString("hh");
    QString minute = t.time().toString("mm");
    QString second = t.time().toString("ss");
    if(m_labUTCmin || second == "00") {
      if((minute.toInt())%2==0) {
        ui->labUTC->setStyleSheet("font-family: MS Shell Dlg 2; font-size: 18pt; background-color: rgb(150,255,255); color : rgb(20,0,177);");
      } else {
        ui->labUTC->setStyleSheet("font-family: MS Shell Dlg 2; font-size: 18pt; background-color: rgb(255,255,150); color : blue;");
      }
      m_labUTCmin = false;
      if((minute.toInt())%5==0 && m_config.usesched() && !m_auto) {
...
void MainWindow::set_scheduler(QString const& setto,bool mixed)
{
  ...
  int frq = 0;
  if (setto.contains(",")) {
    frq = setto.left(setto.indexOf(" ")).replace(",","").toInt();
  } else {
    frq = setto.left(setto.indexOf(" ")).replace(".","").toInt();
  }
  if (mixed) {
...
void MainWindow::stopTx()
{
  ...
  if (m_modeTx=="JT65" or m_modeTx=="JT9" or m_modeTx=="T10")  {
	  dec_data.params.nstophint=0; //let passing through non-CQ messages from Call3 based Hint RX freq decoder 
      stophintTimer.start(314000); //((14+5*60)*1000) block in 5 minutes
   }
  monitor (true);
  QTime currentTime = QTime::currentTime();
  if (currentTime.second() > 0 && currentTime.second() < 35) m_addtx=2; // 2sec is processing delay
  if (currentTime.second() >= 35) dec_data.params.ndelay = 0;
...
void MainWindow::stopTx2()
{
  ...
  if(m_mode=="JT9" and m_bFast9 and m_ntx==5 and (m_nTx73>=5)) {
    on_stopTxButton_clicked();
    m_nTx73=0;
  }
...
void MainWindow::on_dxCheckBox_stateChanged(int ncqdx)        //RRR or RR73
{
  m_cqdx = (ncqdx==2);
  if(!m_cqdx) {
      ui->pbCallCQ->setText("CQ");
    } else {
      ui->pbCallCQ->setText("CQ DX");
    }
}
...
//msgBox (result);
  ...
  // 'replace' workaround to the 'CQ transmitted -> Roger Report received' scenario
  if (dxbasecall == "" && m_qsohistory.at (0).contains ("REPLIEDCQRPRT") && t2.contains (m_baseCall)) {
	if (tcut.contains("R-")) {
		auto index_m = t2.indexOf("R-",22);
		t2.replace(index_m,2,"-");
	} else {
		if (tcut.contains("R+")) {
			auto index_p = t2.indexOf("R+",22);
			t2.replace(index_p,2,"+");
		}
	}
  }

  if (dxbasecall != "" && tcut.contains (dxbasecall) && tcut.contains (m_baseCall)) {
    // 'replace' workaround to the 'GRID TRANSMITTED -> ROGER REPORT RECEIVED' wrong message sequence scenario
    if (m_qsohistory.at (1).contains(dxbasecall + " GRIDTXD") && m_qsohistory.at (0).contains("RRPRTRCVD")) {
		if (tcut.contains("-")) {
			auto index_m = t2.indexOf("R-",22);
			t2.replace(index_m,2,"-");
		} else {
	...
	// 'replace' workaround to the wrong report exchange sequence 'REPORT TXD -> REPORT RCVD' in 'calling CQ' 
	// and 'replying to CQ with report' scenarios
	if (m_qsohistory.at (1).contains(dxbasecall + " REPORTTXD") && m_qsohistory.at (0).contains(" REPORTRCVD")) {
		if (tcut.contains("-")) {
			auto index_m = t2.indexOf("-",22);
			t2.replace(index_m,1,"R-");
		} else {
			if (tcut.contains("+")) {
				auto index_p = t2.indexOf("+",22);
				t2.replace(index_p,1,"R+");
				}
		}
	}
  }
...
//                  printf("auto mode changed %s JT65 %s:%d\n",m_mode.toStdString().c_str(),curBand.toStdString().c_str(),m_pwrBandTxMemory[curBand].toInt());
  ...
  if (m_mode == "JT9+JT65") {
	  if ((decodedtext.isJT9 () and prevmodeTx != "JT9") or
               (decodedtext.isJT65 () and prevmodeTx != "JT65")) TxModeChanged = true;
  }
  ...
  if (i9<0 and !decodedtext.isTX() and m_decodedText2) {
	...
	if (!t2.contains (m_baseCall)) {
		ui->decodedTextBrowser2->displayDecodedText(decodedtext
                                                  ...
                                                  ,m_bypassRxfFilters
                                                  ,m_bypassAllFilters);
	} else if (!m_showMyCallMsgRxWindow) {
			ui->decodedTextBrowser2->displayDecodedText(decodedtext
                                                  ,m_baseCall
                                                  ,m_notified
                                                  ,m_logBook
...
// Determine appropriate response to received message
  ...
  if(dtext.contains (" " + m_baseCall + " ")
     ...
     || (firstcall == "DE" && ((t4.size () > 7 && t4.at(7) != "73") || t4.size () <= 7)))
    {
      ...
      if (t4.size () > 7   // enough fields for a normal msg
        ...
        {
          QString r=t4.at (7);
          if(r.mid(0,3)=="RRR" || (r.toInt()==73) || r.mid(0,4)=="RR73") {
            m_ntx=5;
			m_QSOProgress = SIGNOFF;
            m_nlasttx=5;
            ui->txrb5->setChecked(true);
  ...
  if(m_transmitting) m_restart=true;
  if(m_quick_call) auto_tx_mode(true);
}
...
void MainWindow::genStdMsgs(QString rpt)                       //genStdMsgs()
{
  ...
  if(m_config.my_callsign() !="" and m_config.my_grid() !="")
    {
       if(m_cqdx)
       {     
          t="CQ DX " + m_config.my_callsign() + " " + m_config.my_grid().mid(0,4);
       }
       else
      {
  ...
  QStringList list2 = list1.replaceInStrings("@", hisCall);
...
{
  if (m_autoseq) {
	  m_hisCallOld = m_hisCall;
	  m_hisGridOld = m_hisGrid;
  }
  ui->dxCallEntry->setText("");
  ui->dxCallEntry->setStyleSheet("background-color: rgb(255,255,255);");
  ui->dxGridEntry->setText("");
...
void MainWindow::logClearDX ()
{
  ui->dxCallEntry->setStyleSheet("background-color: rgb(255,255,255);");
  clearDX ();
  m_QSOProgress = CALLING;  // this line is not working, guiupdate very second checking m_QSOProgress based on m_ntx value
  // and there is a conflict with the message buttons in tab2 where m_ntx is always being set to 7
}
...
{
  if(text.length() < 14) ui->tx5->setStyleSheet("background-color: rgb(255,255,255);");
  if(text.length() > 13) ui->tx5->setStyleSheet("background-color: rgb(255,255,0);");
  if(text.length() > 17) ui->tx5->setStyleSheet("background-color: rgb(255,0,0);");
...
void MainWindow::on_wantedCall_textChanged(const QString &wcall) //wanted Call changed
{
  int pos = 0;
  QString call =wcall.toUpper();
  int separate = call.count("/");
  if (separate == 0) {
    if (call_valid.validate(call,pos) != QValidator::Invalid) {
      m_wantedCall=call;
      if(call.length() == 0) ui->wantedCall->setStyleSheet("background-color: rgb(255,255,255);");
      if(call.length() > 0 && call.length() < 3) ui->wantedCall->setStyleSheet("background-color: rgb(255,250,130);");
      if(call.length() > 2) ui->wantedCall->setStyleSheet("background-color: rgb(130,255,140);");
    }
  } else if (separate == 1) {
    separate = call.indexOf("/");
    if (separate != 0) {
      QString call2 = call.mid(separate+1,8);
      if (call_valid.validate(call2,pos) != QValidator::Invalid || separate == call.length() || separate > 3) {
        m_wantedCall=call;
        if(separate <=3 && call2.length() < 3) ui->wantedCall->setStyleSheet("background-color: rgb(255,250,130);");
        if(separate >3 && call2.length() < 1) ui->wantedCall->setStyleSheet("background-color: rgb(255,250,130);");
        if(separate <=3 && call2.length() > 2) ui->wantedCall->setStyleSheet("background-color: rgb(130,255,140);");
        if(separate >3 && call2.length() > 0) ui->wantedCall->setStyleSheet("background-color: rgb(130,255,140);");
...
void MainWindow::on_logQSOButton_clicked()                 //Log QSO button
{
  ...
  if (m_hisCall != "") {
	  ...
	  if(m_autoseq) {
		hisCall = m_hisCallOld;
		hisGrid = m_hisGridOld;
	  }
  }
  auto currenttime = QDateTime::currentDateTimeUtc();
  ...
  if (qAbs(dateTimeQSOOff.toMSecsSinceEpoch() - m_dateTimeQSOOn.toMSecsSinceEpoch()) > m_TRperiod * 10000) m_dateTimeQSOOn = dateTimeQSOOff; // 10min delta
  bool autolog = false;
  if(m_logqso73) autolog = m_config.autolog();

  m_logDlg->initLogQSO (hisCall, hisGrid, m_modeTx, m_rptSent, m_rptRcvd,
...
//  m_logBook.addAsWorked (m_hisCall, m_config.bands ()->find (m_freqNominal), m_modeTx, date, grid);
  QString date = QSO_date_on.toString("yyyyMMdd");
  m_logBook.addAsWorked (call, m_config.bands ()->find (dial_freq), mode, date, grid);
  ...
  ui->dxCallEntry->setStyleSheet("background-color: rgb(127,255,127);");
...
{
  if(m_mode=="T10" or m_mode.mid(0,4)=="WSPR") {
// If coming from T10 or WSPR mode, pretend temporarily that we're coming
...
void MainWindow::on_actionWSPR_2_triggered()
{
  ...
  VHF_features_enabled(false);
  ui->AGCcButton->setVisible(false);
  m_wideGraph->setPeriod(m_TRperiod,m_nsps);
  m_wideGraph->setMode(m_mode);
  m_wideGraph->setModeTx(m_modeTx);
  m_bFastMode=false;
...
void MainWindow::WSPR_config(bool b)
{
  ...
  ui->DecodeButton->setEnabled(!b);
  ui->filterButton->setVisible(!b);
  ui->AGCcButton->setVisible(!b);
  ui->swlButton->setVisible(!b);
  ui->cleanButton->setVisible(!b);
  ui->hintButton->setVisible(!b);
  ui->disableTx73Button->setVisible(!b);
  ui->AutoTxButton->setVisible(!b);
  ui->AutoSeqButton->setVisible(!b);
...
bool MainWindow::reportRCVD(QStringList msg)
{
  ...
  if (msg.length() == 4) { // three words(max index 3) in the message where "CQ DX" is counted as single word
	if (msg.at (1).contains (m_baseCall)
		&& msg.at (2).contains (Radio::base_callsign (ui->dxCallEntry->text ()))
...
{
  if (m_mode.mid(0,4)!="WSPR" and m_mode!="Echo") {
    m_logBook.init(m_config.countryPrefix());    // re-read the log and cty.dat files
...
{
  if(ui->pbSendRRR->hasFocus()) {
     if(event->button() & Qt::RightButton) {
         QString curtext = ui->pbSendRRR->text();
		 if(m_rightbutton != "CQ") {
			m_rightbutton = "CQ";
		    if(!curtext.contains("+CQ")) ui->pbSendRRR->setText(ui->pbSendRRR->text()+"+CQ");
		 } else {
			m_rightbutton = "";
		    if(curtext.contains("+CQ")) {
				if(m_rrr) {
					ui->pbSendRRR->setText("RRR");
				} else {
					ui->pbSendRRR->setText("RR73"); 
		 ...
		 if(!m_transmitting && m_QSOProgressTX == REPORTTX) on_pbSendRRR_clicked();
	 }
// if(event->button() & Qt::RightButton) {
// if(m_rightbutton == "CQ") {
//	m_rightbutton = "";
...
{
  if(text.length() < 14) ui->freeTextMsg->setStyleSheet("background-color: rgb(255,255,255);");
  if(text.length() > 13) ui->freeTextMsg->setStyleSheet("background-color: rgb(255,255,0);");
  if(text.length() > 17) ui->freeTextMsg->setStyleSheet("background-color: rgb(255,0,0);");
      ...
      if (!s.ptt ()) //!m_transmitting)
        {
          ...
          if (m_lastDialFreq != m_freqNominal and m_mode!="JTMSK")
            {
              m_lastDialFreq = m_freqNominal;
              m_repeatMsg=0;
              m_secBandChanged=QDateTime::currentMSecsSinceEpoch()/1000;
              if((s.frequency () < 30000000u || (s.frequency () > 30000000u && !m_config.tx_QSY_allowed ()))  && m_mode.mid(0,4)!="WSPR") {
...
void MainWindow::transmit (double snr)
{
  ...
  if (m_modeTx == "T10") {
    ...
    bool fastmode=false;
    Q_EMIT sendMessage (NUM_T10_SYMBOLS, sps,
                        ui->TxFreqSpinBox->value() - m_XIT, m_toneSpacing,
                        m_soundOutput, m_config.audio_output_channel (),
                        true, fastmode, snr, m_TRperiod);
  }
...
void MainWindow::pskSetLocal ()
{
  ...
  psk_Reporter->setLocalStation(m_config.my_callsign (), m_config.my_grid (),
        antenna_description, QString {"JTDX v18.0"}.simplified ());
}
