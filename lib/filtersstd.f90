! This source code file was last time modified by Igor Chernikov UA3DJY on December 20th, 2016.
...
subroutine filtersstd(decoded,mycall,falsedec,syncpwr)
  ...
  character grid*4,mycall*6,call1*6,call2*6,decoded*22
  logical(1) falsedec
...
! check if any callsign begins with two digits
  ...
  if((grid(1:2).eq."GK" .and. grid(1:4).ne."GK03") .or. &
     ...
     call chkfalse(decoded,falsedec)
     if(falsedec) then
