...
void CountryDat::load(bool displayCountyPrefix)
{
    _data.clear();
    ...
    if (inputFile.open(QIODevice::ReadOnly))
    {
       ...
       while ( !in.atEnd() )
       {
          ...
          if ( !in.atEnd() )
          {
            ...
            QString name;
            if(displayCountyPrefix) {
                name = _extractContinent(line1)+_extractMasterPrefix(line1);
            } else {
                name = _extractContinent(line1)+_extractName(line1);
            }
            if (name.length()>0)
            {
                bool more = true;
