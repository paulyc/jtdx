// This source code file was last time modified by Igor UA3DJY on September 14th, 2017
...
void CPlotter::paintEvent(QPaintEvent *)                                // paintEvent()
{
 ...
 void scale_by_median(float swide[], int* iz)
 {
	 ...
	 if (buf[*iz/2] <= 0) return;
	 for (int i=0; i<*iz; ++i) swide[i] /= buf[*iz/2];
 }
...
//move current data down one line (must do this before attaching a QPainter object)
  ...
  if(bScroll) {
    scale_by_median(swide,&iz);
    flat4_(swide,&iz,&m_Flatten);
    scale_by_median(&dec_data.savg[j0],&jz);
...
//draw tick marks on upper scale
  ...
  if((m_mode=="JT65" or m_mode=="JT9+JT65") and m_modeTx=="JT65" and m_filter==true) {
  QPen pen4(Qt::blue, 2);                 //Mark Filter decoding Freq with blue
  painter0.setPen(pen4);
    x1=XfromFreq(m_rxFreq-200.0);
    x2=XfromFreq(m_rxFreq+200.0);
    painter0.drawLine(x1,23,x1,30);
    painter0.drawLine(x1,23,x2,23);
    painter0.drawLine(x2,23,x2,30);
  QPen pen5(QColor(255,170,127), 4);                 //Mark Filter show Freq with pink
  painter0.setPen(pen5);
    x1=XfromFreq(m_rxFreq-50.0);
    x2=XfromFreq(m_rxFreq+50.0);
    painter0.drawLine(x1,28,x2,28);
    painter0.drawLine(x1,26,x2,26);
  }

  if(m_mode=="T10" and m_filter==true) {
    QPen pen4(Qt::blue, 2);                 //Mark Filter Freq with blue
    painter0.setPen(pen4);
    x1=XfromFreq(m_rxFreq-80.0);
    x2=XfromFreq(m_rxFreq+80.0);
    painter0.drawLine(x1,23,x1,30);
    painter0.drawLine(x1,23,x2,23);
    painter0.drawLine(x2,23,x2,30);
  }

  if(m_mode=="JT9" and m_filter==true) {
    QPen pen5(Qt::blue, 2);                 //Mark Filter Freq with blue
    painter0.setPen(pen5);
    x1=XfromFreq(m_rxFreq-50.0);
    x2=XfromFreq(m_rxFreq+50.0);
    painter0.drawLine(x1,23,x1,30);
  ...
  if(m_mode=="JT9" or m_mode=="T10" or m_mode=="JT65" or m_mode=="JT9+JT65") {
  ...
  if(m_mode=="JT9" or m_mode=="T10" or m_mode=="JT65" or m_mode=="JT9+JT65" or
     m_mode.mid(0,4)=="WSPR") {
    QPen pen1(Qt::red, 3);                   //Mark Tx freq with red
    painter0.setPen(pen1);
  ...
  if(m_mode=="JT9+JT65") {
	...
	if(m_topJT65 < m_fMin) {
		...
		x2=XfromFreq(m_fMin)-3;
		painter0.drawLine(x1,22,x1,28);
		painter0.drawLine(x1,26,x2,26);
		painter0.drawLine(x1,24,x2,24);
	}
  }
...
void CPlotter::setTopJT65(int n)                            //set top JT65 RX freq
{
  m_topJT65=n;
}
